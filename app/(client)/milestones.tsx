  ...
  return {
    ...
    checklist: [
      ...
      },
      {
        ...
        completed: ["awaiting_client_payment", "paid", "completed"].includes(
          milestone.status,
        ),
      },
      {
...
export default function MilestonesScreen() {
  ...
  const milestonesQuery = useQuery({
    ...
    queryFn: async () =>
      (
        await api.get<any[]>(ENDPOINTS.MILESTONES.LIST, {
          params: { projectId },
        })
      ).data,
  });
  ...
  const escrowQuery = useQuery({
    ...
    queryFn: async () =>
      (
        await api.get<{ items: ProjectVault[] }>(
          ENDPOINTS.ESCROW_ACCOUNTS.PROJECT_VAULTS,
        )
      ).data.items,
  });

  const activeEscrow = (escrowQuery.data || []).find(
    (vault) => vault.projectId === projectId,
  );
  ...
  useEffect(() => {
    if (activeProject) {
      setWalletBudget(
        Number(activeProject.budget || activeProject.totalBudget || 0),
      );
    }
    if (activeEscrow) {
  ...
  const [selectedMilestone, setSelectedMilestone] =
    useState<ClientMilestoneView | null>(null);
  const [modalMode, setModalMode] = useState<
    "view" | "passcode" | "revision" | "dispute"
  >("view");
  ...
  const releasePaymentMutation = useMutation({
    mutationFn: async (milestone: ClientMilestoneView) => {
      if (!activeEscrow?.escrowAccountId) {
        throw new Error(
          "No escrow account found for this project. Please fund the project first.",
        );
      }
      const releaseAmount = Number(milestone.budgetAmount || 0);
      if (!Number.isFinite(releaseAmount) || releaseAmount <= 0) {
        throw new Error(
          "This milestone has no payable amount. Add BOQ costs or milestone budget before release.",
        );
      }
  ...
  const handleReleasePaymentPress = async () => {
    ...
    const enrolled = hasHardware
      ...
      : false;
    if (hasHardware && enrolled && selectedMilestone) {
      const result = await LocalAuthentication.authenticateAsync({
        promptMessage: "Confirm milestone payment release",
        fallbackLabel: "Use payment PIN",
        cancelLabel: "Use PIN",
      ...
      if (result.success) {
        try {
          ...
          Alert.alert(
            "Success",
            "Payment released from escrow successfully!  Main Contractor has been notified.",
          );
  ...
  const handleVerifyPasscodeSubmit = async () => {
    ...
    try {
      ...
      setPasscode("");
      setPasscodeError(
        "Could not validate this PIN. Set your payment PIN again from Payments.",
      );
    }
  ...
  const handleDisputeSubmit = async () => {
    ...
    if (disputeDesc.trim().length < 50) {
      Alert.alert(
        "Minimum Length",
        "Please write at least 50 characters to explain the issue clearly for the mediator.",
      );
      return;
    }
      ...
      >
          ...
          >
            {projects.map((p) => (
              <Pressable
                ...
                style={{
                  backgroundColor:
                    projectId === p.id ? COLORS.PRIMARY : COLORS.SURFACE,
        ...
        >
          <Pressable
            ...
            style={{
              ...
              gap: 6,
              backgroundColor:
                viewMode === "timeline" ? COLORS.PRIMARY : "transparent",
              paddingVertical: 10,
              borderRadius: 8,
            }}
          >
          ...
          <Pressable
            ...
            style={{
              ...
              gap: 6,
              backgroundColor:
                viewMode === "board" ? COLORS.PRIMARY : "transparent",
              paddingVertical: 10,
              borderRadius: 8,
            }}
          >
            <Ionicons
              ...
              size={18}
              color={
                viewMode === "board" ? COLORS.TEXT_WHITE : COLORS.TEXT_SECONDARY
              }
            />
            <Text
              style={{
        ...
        {projectMilestones.length === 0 ? (
          ...
          >
            ...
            >
              No milestones found. Milestones will appear once your wallet is
              funded and project is in planning/execution.
            </Text>
          </View>
        ) : viewMode === "timeline" ? (
          // Gantt / Timeline view
          <View style={{ paddingHorizontal: 20, gap: 16 }}>
            {projectMilestones.map((ms, index) => {
              const completedCount = ms.checklist.filter(
                (c) => c.completed,
              ).length;
              const progressPct =
                ms.checklist.length > 0
                  ? (completedCount / ms.checklist.length) * 100
                  : 0;
                  ...
                  <View style={{ gap: 18 }}>
                    ...
                    {selectedMilestone.completionPhotos.length > 0 && (
                      <View>
                        ...
                        >
                          {selectedMilestone.completionPhotos.map(
                            (url, idx) => (
                              <Image
                                key={idx}
                                source={{ uri: url }}
                    ...
                    {selectedMilestone.status === "revision_required" && (
                      ...
                      >
                        ...
                        >
                          Waiting for Main Contractor Resubmission
                        </Text>
                        <Text
                          style={{
                            color: COLORS.TEXT_LIGHT,
                        ...
                        >
                          The Main Contractor has been notified to address your
                ...
                {modalMode === "passcode" && (
                  ...
                  >
                    ...
                    <View style={{ alignItems: "center" }}>
                      ...
                      >
                        Verify your security passcode to approve release of{" "}
                        {formatRWF(selectedMilestone.budgetAmount)} escrow
                        funds.
                      </Text>
                    </View>
                  ...
                  <View style={{ gap: 14 }}>
                    ...
                    >
                      ...
                      <Pressable
                        ...
                        style={[
                          styles.btn,
                          { flex: 2, backgroundColor: COLORS.WARNING },
                        ]}
                      >
                        <Text style={styles.btnText}>
                ...
                {modalMode === "dispute" && (
                  <View style={{ gap: 14 }}>
                    ...
                    <Text
                      style={{ color: COLORS.TEXT_SECONDARY, fontSize: 12 }}
                    >
                      Escrow funds ({formatRWF(selectedMilestone.budgetAmount)})
                      will be frozen in a locked account. The IER arbitrations
                      team will mediate.
                    </Text>
                    ...
                    <View style={{ flexDirection: "row", gap: 6 }}>
                      {(["Quality", "Timeline", "Cost", "Other"] as const).map(
                        (cat) => (
                          <Pressable
                            key={cat}
                    ...
                    >
                      {uploadingEvidence ? (
                        ...
                        <>
                          ...
                          >
                            Attach PDF reports, inspection photos, or bills of
                            quantities.
                          </Text>
                        </>
                      )}
...
function BoardCard({
  ...
  return (
    ...
    >
      <Text
        style={{ color: COLORS.TEXT_PRIMARY, fontWeight: "bold", fontSize: 13 }}
        numberOfLines={1}
      >
        {ms.name}
      </Text>
      <Text
        style={{ color: COLORS.TEXT_SECONDARY, fontSize: 11, marginTop: 4 }}
        numberOfLines={2}
      >
        {ms.description}
      </Text>
      ...
      >
        ...
        <Text
          style={{ fontSize: 11, color: COLORS.PRIMARY, fontWeight: "bold" }}
        >
